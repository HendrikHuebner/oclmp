...
static void computeDataDependencies(vector<Instruction> &instructions, vector<KernelInvocation> &invocations) {
    ...
    for (int i = instructions.size() - 1; i >= 0; i--) {
        Operand x = instructions[i].operand2;
        optional<Operand> y(instructions[i].operand3);
        
        bool found_x = false, found_y = false;

        for (int j = i - 1; j >= 0; j--) {
            Operand z = instructions[j].operand1;
    ...
    for (KernelInvocation kernel : invocations) {
        switch (kernel.inst.type) {
            case InstructionType::Add: {
                kernel.event = oclmp_enqueue_add(env, 1, 
                    kernel.inst.operand2.size(), kernel.inst.operand3.value().size(), kernel.inst.operand1.size(), 
                    kernel.inst.operand2.mem(), kernel.inst.operand3.value().mem(), kernel.inst.operand1.mem(), 
